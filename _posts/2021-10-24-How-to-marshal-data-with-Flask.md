...
This is the sort of response that a GET request to the endpoint would receive (using HTTPie to query a Flask application running locally on port 5000):
...
Marshmallow is a play on words, referring to how it *marshals* data. To do this, it requires that we define schemas, which are outlines of what the data should look like. We then create instances of the schema, which can translate back and forth between JSON and Python data types.  Converting to JSON is *dumping*, and from JSON is *loading*.
...
As you can see, there is a strong similarity between the schema and Flask-RESTful's resource fields. An important difference is that we can use the same Marshmallow schema to get objects both into and out of our application. Loading the JSON returns a Python dictionary, which we can unpack to create a new instance of the Hotel class:
...
Once you have decided to use Marshmallow, there is still another choice to be made between the standard Marshmallow library, and the modified packages specifically designed for Flask web applications.
...
For most applications, it is quicker and easier to use both of these integrated packages.  A few writers advocate [using plain SQLAlchemy](#point_4) instead of Flask-SQLAlchemy for the reason that it makes the database more convenient to access outside the Flask application.  Even if you do decide that you want to do this, Marshmallow-SQLAlchemy works with plain SQLALchemy and you can continue to use it after dropping Flask-Marshmallow and Flask-SQLAlchemy from your project.

### AutoSchema

While having a schema that both loads and dumps has reduced duplication, we still have to write out the same fields that we already have in our models (such as that the hotel name should be a string).

Marshmallow-SQLAlchemy's AutoSchema takes care of this for us:
...
This removes the need for the intermediate step of loading the JSON into dictionaries, and then unpacking those dictionaries into SQLAlchemy models.
...
1. The GET and POST queries to a locally running Flask app were made using [HTTPie](https://httpie.io) in a Bash terminal.
...
4. We need the `/hotels` endpoint so that we can POST new hotels. Including `/hotels` and `/hotels/<int:hotel_id>` in the same Flask-RESTful resource means that calls to both endpoints will be handled by the same controller class. In order to be more RESTful, the controller's GET method should be extended to return all hotels when an ID number is not provided.
